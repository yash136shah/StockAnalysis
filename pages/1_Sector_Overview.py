...
with col2:
    ...
    country_counts= dfC[(dfC[sector].isin(st.session_state["sectorSel"]))]["COUNTRY"].value_counts().to_frame()
    country_list = []
...
issidf=multidfC[(multidfC[sector].isin(st.session_state["sectorSel"])) & (multidfC['COUNTRY'].isin(st.session_state["countrySel"]))]

meandfC = issidf.groupby(by=[sector,"Market Cap Scale"]).mean().sort_values(by=marketCap,ascending=True).reset_index()

sumdfC = issidf.groupby(by=[sector,"Market Cap Scale"]).sum().sort_values(by=marketCap,ascending=True).reset_index()


nco=issidf.groupby(sector)["Market Cap Scale"].value_counts().unstack()
...
with tab2:
    ...
    with col1:
        ...
        add_more_g = st.radio("Add more charts:",("Yes","No","Preselect"),index=1,key="growthaddmore",horizontal=True)
    ...
    with col2:
        ...
        add_more_a = st.radio("Add more charts:",("Yes","No","Preselect"),index=1,key="avgaddmore",horizontal=True)
...
sel_dis_sum = dfF[(dfF[sector].isin(st.session_state["sectorSel"]))&(dfF[sector].isin( st.session_state["sectorSel"]))].groupby(by=[sector,"Market Cap Scale",year]).sum().sort_values(by=marketCap).reset_index()
sel_dis_avg = dfF[(dfF[sector].isin(st.session_state["sectorSel"]))&(dfF[sector].isin( st.session_state["sectorSel"]))].groupby(by=[sector,"Market Cap Scale",year]).mean().sort_values(by=marketCap).reset_index()
...
def DisplayMetric(count=1,st_type=st1,mt_type=rev_type):
        ...
        with col3:
            ...
            if metric_group == "Sum":
                seltdf = sel_dis_sum.pivot_table(index=[sector,"Market Cap Scale"],columns=year,values=metrics,sort=False)
            elif metric_group == "Avg":
                seltdf = sel_dis_avg.pivot_table(index=[sector,"Market Cap Scale"],columns=year,values=metrics,sort=False)
            elif metric_group=="Growth":
                seltdf = sel_dis_sum.pivot_table(index=[sector,"Market Cap Scale"],columns=year,values=metrics,sort=False).pct_change(axis=1)

        


        if metric_group in ["Sum","Avg"]:    
            seltdf.index.names = [sector,"Market Cap Scale"]
            ...
            if bcType == "Grouped":
                    ...
                    for period in periods:
                                    selmtdf = selMtdf.loc[:,[sector,"Market Cap Scale",period]]
                                    x = []
                                    x.append(selmtdf[sector].to_list())
                ...
                fig = px.bar(selMtdf ,x=sector,y=periods,title=metrics,barmode="group",text=mcapText)
            ...
            with col1:
                ...
                selMtdf["sector-Mcap Scale"] = selMtdf[sector].astype("str")+"-"+selMtdf["Market Cap Scale"].astype("str")
                selMtdf.drop([sector,"Market Cap Scale"],axis=1,inplace=True)
                selMtdf.set_index("sector-Mcap Scale",inplace=True)
