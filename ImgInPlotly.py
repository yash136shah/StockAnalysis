...
if name_search == 'Sector & Industry':
    ...
    for el in selection:
        x=el['x']
        y=el['y']
        nsdf=metdfC[(metdfC[x_axis_met]==(x)) & (metdfC[y_axis_met]==(y))]
    ...
    name_selected=st.multiselect("Company Name Selected:",isdfn[coName].unique(),default=name_selected)
    

    df=metdfC[metdfC[coName].isin(name_selected)][["NAME","LOGOURL"]]

def path_to_image_html(path):
    return '<img src="' + path + '" width="60" >'

df = df.drop(df[df["LOGOURL"]==0].index)
